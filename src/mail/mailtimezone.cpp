...
QString QxtTimeZone::abbreviation()
{
#if QT_VERSION < QT_VERSION_CHECK(5,2,0)
...
int QxtTimeZone::tzdToInt(const QString &tzd)
{
	int tzoSign = 1;
	if (tzd.isEmpty() || tzd == QLatin1String("0000")) {
		return 0;
	} else if (tzd.startsWith('+') || tzd.startsWith('-')) {
		QTime time = QTime::fromString(tzd.mid(1), "hhmm");
		if (time.isValid()) {
			if (tzd[0] == '-') {
				tzoSign = -1;
			}
